...
int32_t bezieraux(float px[4][4], float py[4][4], float pz[4][4], int tesselation, FILE* output) {
	...
	float t = 1.0f / tesselation;
		...
		for (float j = 0; j < 1; j += t) {
			...
			coord = std::to_string(x2) + std::to_string(y2) + std::to_string(z2) + std::to_string(re2[0]) + std::to_string(re2[1]) + std::to_string(re2[2]) + std::to_string(j) + std::to_string(i+tesselation);
			write_file(coord, x2, y2, z2, re2[0], re2[1], re2[2],j,i+tesselation, output);
			
			coord = std::to_string(x4) + std::to_string(y4) + std::to_string(z4) + std::to_string(re4[0]) + std::to_string(re4[1]) + std::to_string(re4[2]) + std::to_string(j+tesselation) + std::to_string(i);
			write_file(coord, x4, y4, z4, re4[0], re4[1], re4[2],j+tesselation,i, output);
			
			coord = std::to_string(x2) + std::to_string(y2) + std::to_string(z2) + std::to_string(re2[0]) + std::to_string(re2[1]) + std::to_string(re2[2]) + std::to_string(j) + std::to_string(i+tesselation);
			write_file(coord, x2, y2, z2, re2[0], re2[1], re2[2],j,i+tesselation, output);
			
			coord = std::to_string(x3) + std::to_string(y3) + std::to_string(z3) + std::to_string(re3[0]) + std::to_string(re3[1]) + std::to_string(re3[2]) + std::to_string(j+tesselation) + std::to_string(i+tesselation);
